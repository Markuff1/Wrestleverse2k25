...
const WWEUndisputed: React.FC = () => {
  ...
  const fetchData = async () => {
    const querySnapshot = await getDocs(collection(db, "Wrestleverse", "ChampionshipData", "WWEUndisputed"));
  ...
  const addRecord = async () => {
    if (name && days && date && event) {
      ...
      const docRef = await addDoc(collection(db, "Wrestleverse", "ChampionshipData", "WWEUndisputed"), newRecord);
  ...
  const deleteRecord = async (id: string) => {
    await deleteDoc(doc(db, "Wrestleverse", "ChampionshipData", "WWEUndisputed", id));
    ...
    <div>
      <div className="ChampionshipBackground">
        <div className="ChampionshipContainer">
          <img className="TitleHeaderImage" src="src/Images/CBanners/UCBanner.png" alt="Header Background" />
